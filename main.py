import colorama, requests, capmonster_python,os,sys,time
...
def join(token, invite):
    ...
    try:
        ...
        if "200" not in str(rer):
            ...
            lest.append(f"Created Captcha Task {tt}")
            captcha = cap.join_task_result(tt)
            captcha = captcha["gRecaptchaResponse"]
            lest.append(f"[+] Solved Captcha ")
            ...
            lest.append(f"[+] Joined server {invite_code} With Token")
    except:
        lest.append(f"[-] Failed To Join server {invite_code} With Token")
...
╚══██╔══╝██╔════╝██╔══██╗████╗ ████║██║████╗  ██║██╔══██╗╚══██╔══╝██╔════╝██╔══██╗
   ...
   ██║   ███████╗██║  ██║██║ ╚═╝ ██║██║██║ ╚████║██║  ██║   ██║   ███████╗██████╔╝
   ╚═╝   ╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝╚═╝  ╚═══╝╚═╝  ╚═╝   ╚═╝   ╚══════╝╚═════╝
Github: https://github.com/TTerminated
discord: discord.gg/MCemrFARFy

    """)
    input("[#] Press Enter to continue: ")
    sys.stdout.write(colorama.Fore.CYAN + "--> ")
    invite_code = input("Enter Invite Code (discord.gg/*****): ")
...
sys.stdout.write(colorama.Fore.CYAN + "--> ")
print01("Press Enter To Start Joiner: ")
input("")
print("")
import threading
